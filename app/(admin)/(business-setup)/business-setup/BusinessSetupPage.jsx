...
function BusinessSetupPage() {
  ...
  const complete2ndStep = async (e) => {
    ...
    timeSlots.forEach((slot) => {
      ...
      if (!slot.openingTime.trim()) {
        missingSlots.push(`${period} slot: Opening time is required.`);
      }
      if (!slot.closingTime.trim()) {
        missingSlots.push(`${period} slot: Closing time is required.`);
      }
    });
    if (businessType !== 1 && missingSlots.length > 0) {
      missingSlots.forEach((message) => {
        toast.custom((t) => (
          <AlertToast message={message} dismiss={() => toast.dismiss(t.id)} />
        ));
      });
    } else {
      completeFinalStep(e);
  ...
  function check2ndButtonDisable() {
    return timeSlots.some(
      (slot) => !slot.openingTime.trim() || !slot.closingTime.trim()
    );
  }
                ...
                <div className="center">
                  ...
                  <div className="setup-store-form">
                    <div className="inner-input">
                      <label className="input-label" for="BusinessName">
                        {businessType === 1
                          ? "Store"
                          : businessType === 2
                          ? "Garage"
                    ...
                    <button
                      onClick={complete1stStep}
                      className={`login-btn ${
                        check1stButtonDisable() ? "show_disable_button" : ""
                      }`}
                      disabled={check1stButtonDisable()}
                    >
                      Save & Continue
                    </button>
                  </div>
              ...
              <div className="d-flex justify-content-center">
                <div className="center" style={{ width: "600px" }}>
                  ...
                  <div className="setup-store-form">
                    ...
                    <button
                      className={`login-btn ${
                        check2ndButtonDisable() ? "show_disable_button" : ""
                      }`}
                      onClick={complete2ndStep}
                      disabled={check2ndButtonDisable()}
                    >
                      Save
                    </button>
                  </div>
            ...
            <div className="login-section-inner">
              <div className="d-flex justify-content-center">
                <div className="center">
                  ...
                  <img
                    src="/assets/images/successful.svg"
                    alt="Congratulation"
                  />
