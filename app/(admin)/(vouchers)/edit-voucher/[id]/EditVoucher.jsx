...
export default function EditVoucher({ voucherCreationData, voucher }) {
  ...
  const [selectedExcludeCat, setSelectedExcludeCat] = useState([]);
  const [selectedExcludeBrand, setSelectedExcludeBrand] = useState([]);
  const [selectedExcludeProd, setSelectedExcludeProd] = useState([]);
  ...
  useEffect(() => {
    if (voucher) {
      setFormData({
        ...
        minimumPurchaseAmount: voucher?.minimum_purchase,
        usageLimit: voucher?.usage_limit,
        validFrom: new Date(voucher?.valid_from).toISOString().split('T')[0],
        validTo: new Date(voucher?.valid_to).toISOString().split('T')[0],
  ...
  useEffect(() => {
    //exlcude category
    if (voucher?.exclude_category && categoryOptions.length > 0) {
      const initialSelectedCategories = categoryOptions.filter(option =>
        voucher.exclude_category.includes(option.value)
      );
      setSelectedExcludeCat(initialSelectedCategories);
    }
    //exclude brand
    if (voucher?.exclude_brand && brandOptions.length > 0) {
      const initialSelectedBrands = brandOptions.filter(option =>
        voucher.exclude_brand.includes(option.value)
      );
      setSelectedExcludeBrand(initialSelectedBrands);
    }
    //exlcude product
    if (voucher?.exclude_product && productOptions.length > 0) {
      const initialSelectedProducts = productOptions.filter(option =>
        voucher.exclude_product.includes(option.value)
      );
      setSelectedExcludeProd(initialSelectedProducts);
    }

    //category
    if (voucher?.category && categoryOptions.length > 0) {
      const initialSelectedCategories = categoryOptions.filter(option =>
        voucher.category.includes(option.value)
      );
      setSelectedCat(initialSelectedCategories);
    }
    //brand
    if (voucher?.brand && brandOptions.length > 0) {
      const initialSelectedBrands = brandOptions.filter(option =>
        voucher.brand.includes(option.value)
      );
      setSelectedBrand(initialSelectedBrands);
    }
    //product
    if (voucher?.product && productOptions.length > 0) {
      const initialSelectedProducts = productOptions.filter(option =>
        voucher.product.includes(option.value)
      );
  ...
  const handleMultiSelectChange = (selectedOptions, fieldName) => {
    // If no options are selected, set selectedValues to an empty array
    ...
    if (fieldName === 'excludeBrandIds') {
      setSelectedExcludeBrand(selectedOptions || []);
    }
    // Update the selected state for exclude product
    if (fieldName === 'excludeProductIds') {
      setSelectedExcludeProd(selectedOptions || []);
    }

    // Update the selected state for exclude categories
    if (fieldName === 'categoriesIds') {
      setSelectedCat(selectedOptions || []);
    }
    // Update the selected state for exclude brands
    if (fieldName === 'brandIds') {
      setSelectedBrand(selectedOptions || []);
    }
    // Update the selected state for exclude product
    if (fieldName === 'productIds') {
      setSelectedProd(selectedOptions || []);
    }
  };
  ...
  const handleSubmit = async e => {
    ...
    // console.log("formDataInSubmit:", formData); return;
    ...
    try {
      const response = await axiosInstance.put(
        `/seller-panel-api/frontend/update-voucher/${voucher?.id}/`,
        {
          ...formData,
        }
      );
